    ...
    * Optional mid-course decoy deployment with configurable seeker confusion
    ...
    if interceptors is None:
        interceptors = [
            ...
            InterceptorConfig(
                name="THAAD",
                site=(interceptor_site[0] + 1_600_000.0, 0.0),
                launch_delay=interceptor_launch_delay + 520.0,
                engage_altitude_min=0.0,
                engage_altitude_max=160_000.0,
                speed_cap=3200.0,
                guidance_gain=max(0.4, guidance_gain * 1.15),
                damping_gain=max(0.08, damping_gain * 1.5),
                intercept_distance=220.0,
                max_accel=max(interceptor_max_accel, 110.0),
                guidance_noise_std_deg=max(0.05, guidance_noise_std_deg * 1.2),
