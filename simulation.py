    ...
    * Configurable interceptor salvos (count and spacing) so layered defenses can
    ...
    if interceptors is None:
        interceptors = [
            ...
            InterceptorConfig(
                name="THAAD",
                site=(interceptor_site[0] + 3_200_000.0, 0.0),
                launch_delay=interceptor_launch_delay + 260.0,
                engage_altitude_min=0.0,
                engage_altitude_max=400_000.0,
                speed_cap=5800.0,
                guidance_gain=max(0.7, guidance_gain * 1.45),
                damping_gain=max(0.11, damping_gain * 1.85),
                intercept_distance=150_000.0,
                max_accel=max(interceptor_max_accel, 200.0),
                guidance_noise_std_deg=max(0.03, guidance_noise_std_deg * 1.02),
                confusion_probability=max(0.0, min(1.0, decoy_confusion_probability * 0.75)),
                reacquisition_rate=max(decoy_reacquisition_rate * 2.2, 0.04),
                max_flight_time=700.0,
                depends_on="GBI",
                dependency_grace_period=60.0,
