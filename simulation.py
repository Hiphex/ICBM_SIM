    ...
    if interceptors is None:
        interceptors = [
            ...
            InterceptorConfig(
                ...
                speed_cap=5200.0,
                guidance_gain=max(0.68, guidance_gain * 1.4),
                damping_gain=max(0.1, damping_gain * 1.8),
                intercept_distance=60_000.0,
                max_accel=max(interceptor_max_accel, 180.0),
                guidance_noise_std_deg=max(0.032, guidance_noise_std_deg * 1.05),
                confusion_probability=max(0.0, min(1.0, decoy_confusion_probability * 0.78)),
                reacquisition_rate=max(decoy_reacquisition_rate * 1.9, 0.035),
                max_flight_time=620.0,
                depends_on="GBI",
